...
{
    public const ushort Join = (ushort)(((int)MessageDomain.Session << MessageBits.SHIFT_DOMAIN)
        | (MessageBits.DIR_REQ << MessageBits.SHIFT_DIR)
        | (MessageBits.DefaultVersion << MessageBits.SHIFT_VER)
        | 1);

    public const ushort Allocated = (ushort)(((int)MessageDomain.Session << MessageBits.SHIFT_DOMAIN)
