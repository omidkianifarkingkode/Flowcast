...
public sealed class InMemorySessionRepository : ISessionRepository
{
    ...
    public Task<Result<Session>> GetById(SessionId id, CancellationToken ct = default)
    {
        if (_store.TryGetValue(id, out var session))
            return Task.FromResult(Result.Success(session));

        return Task.FromResult(Result.Failure<Session>(SessionErrors.SessionNotFound));
    }

    public Task<Result<IReadOnlyList<Session>>> GetAll(CancellationToken ct = default)
    {
        // Snapshot to avoid enumeration over a changing dictionary
        var list = _store.Values.ToArray();
        return Task.FromResult(Result.Success((IReadOnlyList<Session>)list));
    }

    public Task<Result<Session?>> GetActiveByPlayer(PlayerId playerId, CancellationToken ct = default)
    {
        if (_activeByPlayer.TryGetValue(playerId, out var sid) &&
            _store.TryGetValue(sid, out var session) &&
            IsActive(session))
        {
            return Task.FromResult(Result.Success<Session?>(session));
        }
        return Task.FromResult(Result.Success<Session?>(null));
    }
    ...
    private static bool IsActive(Session s)
